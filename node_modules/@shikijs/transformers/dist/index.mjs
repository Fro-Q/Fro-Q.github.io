...
function transformerMetaHighlight(options = {}) {
  ...
  return {
    ...
    line(node, line) {
      ...
      console.log(this.options.meta.__raw)
...
function transformerNotationMap(options = {}, name = "@shikijs/transformers:notation-map") {
  ...
  return createCommentNotationTransformer(
    ...
    function ([_, match, range = ':1'], _line, _comment, lines, index) {
      let [startLine, endLine] = [index, index]

      if (range) {
        const rangeParts = range.slice(1).split('-')
        if (rangeParts.length === 2) {
          [startLine, endLine] = rangeParts.map(part => Number.parseInt(part, 10) + index - 1)
          if (startLine < index) return false
        }
        else {
          endLine = Number.parseInt(rangeParts[0], 10)
        }
      }
